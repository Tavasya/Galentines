...
export default function DateSetup() {
  ...
  // Step 2: Once linear reaches 100, start animating the circular bar
  useEffect(() => {
    ...
    if (progress >= 100) {
      ...
      interval = setInterval(() => {
        setValue((prev) => {
          // Once circular reaches 100, stop.
          if (prev >= 100) {
  ...
  // Define an onEnter handler that uses the router.
  // This function is only called when the Terminal component is mounted and the user presses Enter.
  const handleEnter = () => {
    // Optionally, you can check if router.isReady (if using next/router)
    router.push("/end");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center">
      ...
      {/* Once both progress bars are done, show the terminal output */}
      {!showLinearProgress && !showCircularProgress && (
        <Terminal onEnter={handleEnter} className="bg-[#1C1018]">
          <TypingAnimation className="font-extrabold text-white">
            &gt; npm install dateConfig@latest init
          </TypingAnimation>
